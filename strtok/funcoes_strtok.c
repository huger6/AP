...
void remover_espacos(char * str) {
    ...
    if (inicio != str) {
        ...
        //se dest > src copia do fim para o inicio
        //se dest < src copia do inicio para o fim
        //copia n bytes
        //Neste caso, inicio pode ser > str, logo copia para trás
    }
...
void carregar_dados(const char * nome_ficheiro_dados,const char * nome_ficheiro_escolar, Estudante ** aluno, int * tamanho_alunos, Dados ** escolares, int * tamanho_escolares) { 
    ...
    if(dados) { //apenas se a abertura tiver sido bem sucedida
        while ((linha = ler_linha(dados, &n_linhas)) != NULL) {
            ...
            char * parte = strtok(linha, SEPARADOR); //Vai buscar a primeira parte da linha, até ao separador
            while (parte != NULL) {
                parametros[num_parametros] = malloc(strlen(parte) + 1); //Aloca memória para a string
                if (parametros[num_parametros] != NULL) {
                    strcpy(parametros[num_parametros], parte); //Copia o conteúdo para a string
                    num_parametros++;
                }
                else {
                    printf("Erro!\n");
                ...
                parte = strtok(NULL, SEPARADOR); //Vai buscar a próxima parte (NULL para continuar a partir do último ponto)
            }
